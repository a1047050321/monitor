...
var util = {
    ...
    /**
     * 处理时间函数 转为utc时间
     * @param  {Function}  date 传回时间 new Date()
...
var methods = {
     ...
     */
    logout: function() {
        axios.get(iHomed("api", "login_out")).then((res) => {
    ...
    nodeType: function(data) {
        ...
        for (let i = 0; i < data.length; i++) {
            if (data[i].type == 1) {
                self.communityLabel = data[i].label;
                self.areaId = data[i].id;
                self.first = $.extend({}, data[i]);
                flag = true;
                break;
            } else {
                flag = Array.isArray(data[i].children) && self.nodeType(data[i].children);
                if (flag) {
                    break;
                }
            }
    ...
    http: function(options) {
        ...
        //设置请求数据中的accesstoken
        // if (!options.data.Authorization) {
        // options.data.Authorization = CONFIG.token;
        // }
    ...
    response => {
        if (response.data.code == 4002 || response.data.code == 401 || response.data.code == 403 || response.data.code == 415 ) {
            ...
            if (router.history.current.name == "Login") {
                ...
                // alert(router.history.current.name);
...
export default {
    ...
    install: function(vm) {
        ...
        vm.prototype.timeStamp = methods.timeStamp;

...
<div>
    <div class="monitorInfo" v-if=" !row.monitor && row.name">
     ...
     <div class="monitorInfo" v-show=" row.monitor && tableData.length != 0">
        <div class="infoHeader">报警信息</div>
        <div class="pending">待处理:{{i+1 +"/"+length}}</div>
             <div class="currInfo"></div>
        <ul class="opeator">
            <li class="detail">
            <ul>
                <li style="width:190px; overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:14px;"><span>地区:{{row.address}}</span></li>
                <br/>
            ...
            <router-link :to="{path:'/handlePending',query:{row:JSON.stringify(row)}}">
                <div><el-button type="primary" @click="ulbtnClick" style="position:absolute;top:136px;right:140px;">处理</el-button></div>
            </router-link>     
            </li>
            <li class="next">
                <div><el-button type="primary" @click="nextInfo" style="position:absolute;top:136px;right:40px;">下一条</el-button></div>
            </li>
        </ul>
         <no-monitor :mode="mode" :row="row" v-show="liClick" @liClick="opeator" @buttonShow="infoChange"></no-monitor>
        <edital-info v-show="editalInfo" @editalInfo="editalInfoReturn" :row="row" :alarmType="alarmType"></edital-info>
   ...
   <div class="monitorInfo" v-show="!buttonShow">
                <span class="icon-success" style="position:absolute;left:120px;top:60px;"></span>
